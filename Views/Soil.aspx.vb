Public Class Soil
            ...
            For j = 0 To _fieldsInfo1.Count - 1
                For i = 0 To gvSoils.Rows.Count - 1
                    If j = currentFieldNumber Then
                        ...
                        _fieldsInfo1(currentFieldNumber).UpdateSoil(tTXTKey.Value, tTXTSymbol.Value, tTXTGroup.Value, tTXTName.Value, tTXTAlbedo.Value, tTXTSlope.Value, tTXTPercentage.Value, i, tCHKSelecte.Checked, tDDLdrainageCl.Items(tDDLdrainageCl.SelectedIndex).Value)
    ...
    Private Sub ChangeLanguageContent()
        ...
        gvSoils.Columns(10).HeaderText = cntDoc.Descendants("SoilDrainageClass").Value
    ...
    Private Sub LoadSoils()
        If _startInfo.Status = userInput Or ddlSSA.SelectedIndex < 0 Then Exit Sub
    ...
    Private Sub LoadDataLayers(soilNumber As Short)
        ...
        'If _fieldsInfo1(currentFieldNumber)._soilsInfo(currentSoilNumber)._layersInfo(0).Depth > (3.95) Then
        '    With _fieldsInfo1(currentFieldNumber)._soilsInfo(currentSoilNumber)._layersInfo(0)
        '        Dim first_layer As New LayersData
        '        first_layer.LayerNumber = 0
        '        first_layer.Depth = Math.Round(10 / in_to_cm, 2)
        '        first_layer.Sand = .Sand
        '        first_layer.Silt = .Silt
        '        first_layer.OM = .OM
        '        first_layer.BD = .BD
        '        first_layer.PH = .PH
        '        first_layer.Cec = .Cec
        '        first_layer.SoilP = .SoilP
        '        _fieldsInfo1(currentFieldNumber)._soilsInfo(currentSoilNumber)._layersInfo.Insert(0, first_layer)
        '    End With
        'End If
    ...
    Private Sub gvLayers_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvLayers.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim txtSoilP As New HtmlInputText
            ...
            txtSoilP = e.Row.Cells(8).FindControl("txtSoilP")

            'If Single.Parse(txtDepth.Value) > 8 And e.Row.RowIndex > 0 Then txtSoilP.Disabled = True
